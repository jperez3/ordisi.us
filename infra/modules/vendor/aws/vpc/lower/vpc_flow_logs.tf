...
# S3 access logging is intentionally disabled to avoid recursive logging on this log-only bucket.
...
# SSE-S3 is sufficient for these low-sensitivity operational logs; a customer-managed KMS key would add cost and administration.
